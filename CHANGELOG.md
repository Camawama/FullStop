...
- **`fullstop:bouncing_projectiles` entity-type tag** (empty by default): opt modded projectiles (grappling hooks and the like) into FullStop's collision physics — they bounce off slime like everything else, redirected by the pre-contact lookahead before their own impact handling fires.
